...
<head>
	...
	<title>IT HILLSTATE - Proposal</title>
	<!-- Favicon-->
...
<body class="d-flex flex-column min-vh-100">
	...
	<h3>건의함</h3>
		<article>	
			<form style="width: 60%; float: right;" action="proposal_search.jsp" method="get">
...
ArrayList<ProposalBoardDTO> boardList = dao.selectBoardList(startRow, listLimit);
...
for(ProposalBoardDTO board : boardList) {
ReplyDAO reply = new ReplyDAO();
int count = reply.replyCount(board.getNum(), "proposal");
%>
			<!-- 반복문 내에서 각 레코드를 각 행에 표시 -->
			<!-- 	<tbody class="table-striped"> -->
				<tr onclick="location.href='proposal_content.jsp?num=<%=board.getNum()%>'"  style="cursor:pointer;">
					<th scope="row"><%=board.getNum() %></th>
					<td class="left" style="text-align: left;"><%=board.getSubject() %>  <%if(count > 0) {%><h7 style="font-weight: bold; display: inline-block;"> [<%=count %>]</h7><%} %></td>
					<td><%=board.getName() %></td>
...
%>
			...
			<div>
				<input type="button" value="글쓰기" class="btn btn-secondary" onclick="location.href='proposal_write.jsp'" style="float: right;">
			</div>
		</article>
	<br><br>
		<div style="text-align: center;">
			<ul class="pagination" style="display: inline-block;">
				...
				<%} else { %>
					<li class="page-item" style="display: inline-block;">				
						<a class="page-link" href="proposal.jsp?pageNum=<%=pageNum - 1%>" style="color: black;">&laquo;</a>
					</li>
				<%} %>
				...
				<%for(int i = startPage; i <= endPage; i++) { %>
					...
					<%} else { %>
						<a class="page-link" href="proposal.jsp?pageNum=<%=i %>" style="color: black;"><%=i %></a>
					<%} %>
						</li>
				<%} %>
				...
				<%} else { %>
					<li class="page-item" style="display: inline-block;">
						<a class="page-link" href="proposal.jsp?pageNum=<%=pageNum + 1%>" style="color: black;">&raquo;</a>
					</li>
				<%} %>
			</ul>
		</div>
