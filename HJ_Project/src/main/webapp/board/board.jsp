...
<head>
	...
	<title>IT HILLSTATE - Board</title>
	<!-- Favicon-->
...
<body class="d-flex flex-column min-vh-100">
	...
	<h3>자유게시판</h3>
		<article>	
			<form style="width: 60%; height: 20px; float: right;" action="board_search.jsp" method="get">
...
int count = reply.replyCount(board.getNum(), "board");

%>
				<!-- 반복문 내에서 각 레코드를 각 행에 표시 -->
				<!-- 	<tbody class="table-striped"> -->
				<tr onclick="location.href='board_content.jsp?num=<%=board.getNum()%>'" style="cursor:pointer;">
...
%>
			...
			<div>
				<input type="button" value="글쓰기" class="btn btn-secondary" onclick="location.href='board_write.jsp'" style="float: right;">
			</div>
		</article>
			<br><br>
			<div style="text-align: center;">
				<ul class="pagination" style="display: inline-block;">
					...
					<%} else { %>
						<li class="page-item" style="display: inline-block;">				
							<a class="page-link" href="board.jsp?pageNum=<%=pageNum - 1%>" style="color: black;">&laquo;</a>
						</li>
					<%} %>
					...
					<%for(int i = startPage; i <= endPage; i++) { %>
						...
						<%} else { %>
							<a class="page-link" href="board.jsp?pageNum=<%=i %>" style="color: black;"><%=i %></a>
						<%} %>
							</li>
					<%} %>
					...
					<%} else { %>
						<li class="page-item" style="display: inline-block;">
							<a class="page-link" href="board.jsp?pageNum=<%=pageNum + 1%>" style="color: black;">&raquo;</a>
						</li>
					<%} %>
				</ul>
			</div>
