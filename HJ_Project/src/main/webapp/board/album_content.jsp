...
AlbumBoardDTO board = dao.selectAlbumBoard(num);
...
<head>
	...
	<title>IT HILLSTATE - Album Content</title>
	<!-- Favicon-->
...
<script type="text/javascript">
	function forwardReply() {
		...
		<%if(sId != null) {%>
			...
			location.href = "album_content_reply_writePro.jsp?boardType=album&num=<%=board.getNum()%>&name=<%=sId%>&content=" + content;
		<%} else {%>
		...
		<article>
			<table class="table" style="margin-top: 50px;">
				...
				<tr>
					...
					<td colspan="3" style="height: 200px; text-align: left;"><img src="../imgupload/<%=board.getOriginal_imgfile() %>" style="width: 500px;"><br><br><%=board.getContent() %></td>
				</tr>
				<tr>
					<td>파일</td>
					<td colspan="3" style="text-align: left;">
						...
						-->
						<a href="../imgupload/<%=board.getReal_imgfile()%>" download="<%=board.getOriginal_imgfile() %>"><%=board.getOriginal_imgfile() %></a>
					</td>
				</tr>
				<tr>
				<!-- 댓글 시작 -->
...
// 				List<ReplyDTO> replyList = replyDAO.selectReplyList(num);
				...
				for(ReplyDTO reply : replyList) {
					...
					<tr>
						...
						<td>
							...
							<button class ="btn btn-outline-secondary btn-sm" style="cursor:pointer; margin-right: 40px;" onclick="location.href='album_content_reply_deletePro.jsp?num=<%=reply.getNum()%>&refNum=<%=num%>'">
			...
			<div id="table_search">
				<input type="button" value="수정" class="btn btn-secondary" onclick="location.href='album_update.jsp?num=<%=num%>'">
				<input type="button" value="삭제" class="btn btn-secondary" onclick="location.href='album_delete.jsp?num=<%=num%>'"> 
				<input type="button" value="목록" class="btn btn-secondary" onclick="location.href='album.jsp'">
			</div>
		</article>
	</main>  
	<!-- 푸터 들어가는곳 -->
