...
<head>
	...
	<title>IT HILLSTATE - Download Content</title>
	<!-- Favicon-->
...
<script type="text/javascript">
	function forwardReply() {
		...
		<%if(sId != null) {%>
			...
			location.href = "download_content_reply_writePro.jsp?boardType=download&num=<%=board.getNum()%>&name=<%=sId%>&content=" + content;
		<%} else {%>
...
<body class="d-flex flex-column min-vh-100">
	...
	<main class="container-sm">
		<article>
				...
				for(ReplyDTO reply : replyList) {
					...
					<tr>
						...
						<td>
							...
							<button class ="btn btn-outline-secondary btn-sm" style="cursor:pointer; margin-right: 40px;" onclick="location.href='download_content_reply_deletePro.jsp?num=<%=reply.getNum()%>&refNum=<%=num%>'">
			...
			<div id="table_search">
				<input type="button" value="수정" class="btn btn-secondary" onclick="location.href='download_update.jsp?num=<%=num%>'">
				<input type="button" value="삭제" class="btn btn-secondary" onclick="location.href='download_delete.jsp?num=<%=num%>'"> 
				<input type="button" value="목록" class="btn btn-secondary" onclick="location.href='download.jsp'">
			</div>
		</article>
	</main>  
	<!-- 푸터 들어가는곳 -->
