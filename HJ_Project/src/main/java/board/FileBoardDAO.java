...
public class FileBoardDAO {
	...
	public int selectBoardListCount(String searchSelect, String keyword) {
		...
		try {
			...
			// => WHERE 절에 LIKE 결합
			String sql = "SELECT COUNT(*) FROM file_board WHERE "+ searchSelect + " LIKE ?";
			pstmt = con.prepareStatement(sql);
	...
	public ArrayList<FileBoardDTO> selectBoardList(int startRow, int listLimit, String searchSelect, String keyword) {
		ArrayList<FileBoardDTO> boardList = null;
		...
		try {
			...
			String sql = "SELECT * FROM file_board WHERE " + searchSelect + " LIKE ? ORDER BY num DESC LIMIT ?,?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, "%" + keyword + "%"); // 검색어를 포함하기 위해 % 결합
