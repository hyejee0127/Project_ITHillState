...
public class ProposalBoardDAO {
	...
	public int selectBoardListCount(String searchSelect, String keyword) {
		...
		try {
			...
			// => WHERE 절에 LIKE 결합
			String sql = "SELECT COUNT(*) FROM proposal_board WHERE "+ searchSelect + " LIKE ?";
			pstmt = con.prepareStatement(sql);
	...
	public ArrayList<ProposalBoardDTO> selectBoardList(int startRow, int listLimit) {
		// ArrayList 타입 사용 시 클래스명 뒤에 <> 기호를 쓰고, 기호 사이에 클래스타입을 지정하면
		// 해당 ArrayList 객체에서 사용되는 데이터는 모두 지정된 클래스타입 데이터로만 사용된다!
		ArrayList<ProposalBoardDTO> boardList = null; // ArrayList 에서 사용할 데이터를 BoardDTO 타입으로 고정
		...
		try {
			...
			String sql = "SELECT * FROM proposal_board ORDER BY num DESC LIMIT ?,?";
			pstmt = con.prepareStatement(sql);
			...
			boardList = new ArrayList<ProposalBoardDTO>(); // BoardDTO 객체 저장 전용 ArrayList 객체 생성됨
			
			// 조회 결과가 있을 경우 레코드 접근 반복
			while(rs.next()) {
				// 1개 레코드를 저장할 BoardDTO 객체 생성 후 데이터 저장
				ProposalBoardDTO board = new ProposalBoardDTO();
	...
	public ArrayList<ProposalBoardDTO> selectBoardList(int startRow, int listLimit, String searchSelect, String keyword) {
		ArrayList<ProposalBoardDTO> boardList = null; // ArrayList 에서 사용할 데이터를 BoardDTO 타입으로 고정
		...
		try {
			...
			String sql = "SELECT * FROM proposal_board WHERE " + searchSelect + " LIKE ? ORDER BY num DESC LIMIT ?,?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, "%" + keyword + "%"); // 검색어를 포함하기 위해 % 결합
	...
	public ArrayList<ProposalBoardDTO> selectRecentBoardList() {
		ArrayList<ProposalBoardDTO> boardList = null; // ArrayList 에서 사용할 데이터를 BoardDTO 타입으로 고정
		...
		try {
			...
			String sql = "SELECT * FROM proposal_board ORDER BY num DESC LIMIT 5";
			pstmt = con.prepareStatement(sql);
	...
	public int insertProposalBoard(ProposalBoardDTO board) {
		int insertCount = 0;
		...
		try {
			...
			String sql = "SELECT MAX(num) FROM proposal_board";
			pstmt = con.prepareStatement(sql);
			...
			// 3단계. SQL 구문 작성 및 전달
			// => 글 쓰기(INSERT) 작업 수행
			sql = "INSERT INTO proposal_board VALUES (?,?,?,?,?,?,now())";
	...
	public void updateReadcount(int num) {
		...
		try {
			...
			String sql = "UPDATE proposal_board SET readcount=readcount+1 WHERE num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public ProposalBoardDTO selectProposalBoard(int num) {
		ProposalBoardDTO board = null;
		...
		try {
			String sql = "SELECT * FROM proposal_board WHERE num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			...
			System.out.println("SQL 구문 오류 발생! - selectProposalBoard()");
			e.printStackTrace();
		} finally {
			// DB 자원 반환
			JdbcUtil.close(rs);
	...
	public int deleteProposalBoard(int num, String pass) {
		int deleteCount = 0;
		...
		try {
			...
			String sql = "DELETE FROM proposal_board WHERE num=? AND pass=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public int updateProposalBoard(ProposalBoardDTO board) {
		int updateCount = 0;
		...
		try {
			...
			String sql = "UPDATE proposal_board SET name=?,subject=?,content=? WHERE num=? AND pass=?";
			pstmt = con.prepareStatement(sql);
