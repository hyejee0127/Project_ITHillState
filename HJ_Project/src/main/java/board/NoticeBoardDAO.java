...
public class NoticeBoardDAO {
	...
	public int selectBoardListCount(String searchSelect, String keyword) {
		...
		try {
			...
			// => WHERE 절에 LIKE 결합
			String sql = "SELECT COUNT(*) FROM notice_board WHERE "+ searchSelect + " LIKE ?";
			pstmt = con.prepareStatement(sql);
	...
	public ArrayList<NoticeBoardDTO> selectBoardList(int startRow, int listLimit) {
		// ArrayList 타입 사용 시 클래스명 뒤에 <> 기호를 쓰고, 기호 사이에 클래스타입을 지정하면
		// 해당 ArrayList 객체에서 사용되는 데이터는 모두 지정된 클래스타입 데이터로만 사용된다!
		ArrayList<NoticeBoardDTO> boardList = null; // ArrayList 에서 사용할 데이터를 BoardDTO 타입으로 고정
		...
		try {
			...
			String sql = "SELECT * FROM notice_board ORDER BY num DESC LIMIT ?,?";
			pstmt = con.prepareStatement(sql);
			...
			boardList = new ArrayList<NoticeBoardDTO>(); // BoardDTO 객체 저장 전용 ArrayList 객체 생성됨
	...
	public ArrayList<NoticeBoardDTO> selectBoardList(int startRow, int listLimit, String searchSelect, String keyword) {
		ArrayList<NoticeBoardDTO> boardList = null; // ArrayList 에서 사용할 데이터를 BoardDTO 타입으로 고정
		...
		try {
			...
			String sql = "SELECT * FROM notice_board WHERE " + searchSelect + " LIKE ? ORDER BY num DESC LIMIT ?,?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, "%" + keyword + "%"); // 검색어를 포함하기 위해 % 결합
	...
	public ArrayList<NoticeBoardDTO> selectRecentBoardList() {
		ArrayList<NoticeBoardDTO> boardList = null; // ArrayList 에서 사용할 데이터를 BoardDTO 타입으로 고정
	...
	public int insertNoticeBoard(NoticeBoardDTO board) {
		int insertCount = 0;
	...
	public NoticeBoardDTO selectNoticeBoard(int num) {
		...
		try {
			...
			System.out.println("SQL 구문 오류 발생! - selectNoticeBoard()");
			e.printStackTrace();
		} finally {
			// DB 자원 반환
			JdbcUtil.close(rs);
	...
	public int deleteNoticeBoard(int num, String pass) {
		int deleteCount = 0;
	...
	public int updateNoticeBoard(NoticeBoardDTO board) {
		int updateCount = 0;
