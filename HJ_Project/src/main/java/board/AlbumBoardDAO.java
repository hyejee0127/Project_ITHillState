...
public class AlbumBoardDAO {
	...
	public int selectBoardListCount() {
		...
		try {
			...
			String sql = "SELECT COUNT(*) FROM album_board";
	...
	public int selectBoardListCount(String searchSelect, String keyword) {
		...
		try {
			...
			// => WHERE 절에 LIKE 결합
			String sql = "SELECT COUNT(*) FROM album_board WHERE "+ searchSelect + " LIKE ?";
			pstmt = con.prepareStatement(sql);
	...
	public ArrayList<AlbumBoardDTO> selectBoardList(int startRow, int listLimit) {
		ArrayList<AlbumBoardDTO> boardList = null;
		...
		try {
			...
			String sql = "SELECT * FROM album_board ORDER BY num DESC LIMIT ?,?";
			pstmt = con.prepareStatement(sql);
			...
			boardList = new ArrayList<AlbumBoardDTO>();
			...
			while(rs.next()) {
				...
				board.setOriginal_imgfile(rs.getString("original_imgfile"));
				board.setReal_imgfile(rs.getString("real_imgfile"));
	...
	public ArrayList<AlbumBoardDTO> selectBoardList(int startRow, int listLimit, String searchSelect, String keyword) {
		ArrayList<AlbumBoardDTO> boardList = null;
		...
		try {
			...
			String sql = "SELECT * FROM album_board WHERE " + searchSelect + " LIKE ? ORDER BY num DESC LIMIT ?,?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, "%" + keyword + "%"); // 검색어를 포함하기 위해 % 결합
	...
	public int insertAlbumBoard(AlbumBoardDTO board) {
		int insertCount = 0;
		...
		try {
			...
			String sql = "SELECT MAX(num) FROM album_board";
			pstmt = con.prepareStatement(sql);
			...
			// 3단계. SQL 구문 작성 및 전달
			// => 글 쓰기(INSERT) 작업 수행
			sql = "INSERT INTO album_board VALUES (?,?,?,?,?,?,?,?,now())";
			...
			pstmt2.setString(6, board.getOriginal_imgfile());
			pstmt2.setString(7, board.getReal_imgfile());
	...
	public void updateReadcount(int num) {
		...
		try {
			...
			String sql = "UPDATE album_board SET readcount=readcount+1 WHERE num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public AlbumBoardDTO selectAlbumBoard(int num) {
		AlbumBoardDTO board = null;
		...
		try {
			String sql = "SELECT * FROM album_board WHERE num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public String selectRealFile(int num) {
		...
		try {
			String sql = "SELECT real_imgfile FROM album_board WHERE num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			...
			if(rs.next()) {
				realFile = rs.getString("real_imgfile");
			}
		}  catch (SQLException e) {
	...
	public int deleteAlbumBoard(int num, String pass) {
		int deleteCount = 0;
		...
		try {
			...
			String sql = "DELETE FROM album_board WHERE num=? AND pass=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public int updateAlbumBoard(AlbumBoardDTO board) {
		int updateCount = 0;
		...
		try {
			...
			// 글번호와 패스워드가 일치하는 레코드 삭제
			String sql = "UPDATE album_board " +
							"SET subject=?,content=?,original_imgfile=?,real_imgfile=?" +
							"WHERE num=? AND pass=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, board.getSubject());
			pstmt.setString(2, board.getContent());
			pstmt.setString(3, board.getOriginal_imgfile());
			pstmt.setString(4, board.getReal_imgfile());
